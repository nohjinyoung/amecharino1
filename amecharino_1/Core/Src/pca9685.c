...
void PCA9685_SetBit(uint8_t Register, uint8_t Bit, uint8_t Value)
{
  ...
  HAL_I2C_Mem_Read(&hi2c3, PCA9685_ADDRESS, Register, 1, &readValue, 1, 10);
  if (Value == 0) readValue &= ~(1 << Bit);
  else readValue |= (1 << Bit);
  HAL_I2C_Mem_Write(&hi2c3, PCA9685_ADDRESS, Register, 1, &readValue, 1, 10);
  HAL_Delay(1);
}
...
void PCA9685_SetPWMFrequency(uint16_t frequency)
{
  ...
  HAL_I2C_Mem_Write(&hi2c3, PCA9685_ADDRESS, PCA9685_PRE_SCALE, 1, &prescale, 1, 10);
...
void PCA9685_SetPWM(uint8_t Channel, uint16_t OnTime, uint16_t OffTime)
{
  ...
  pwm[2] = OffTime & 0xFF;
  pwm[3] = OffTime>>8;
  HAL_I2C_Mem_Write(&hi2c3, PCA9685_ADDRESS, registerAddress, 1, pwm, 4, 10);
}
